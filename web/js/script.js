...
// edit post
    $('.edit-post').on('click', function () {
        $('.editPost').addClass('active');
        return false;
    });
    $('.edit-closed, .cancel').on('click', function () {
        $('.editPost').removeClass('active');
        return false;
    });
// delete Friend
    $('.delete-friend').on('click', function () {
        $('.popup-wraper8').addClass('active');
        return false;
    });
    $('.deleteFriend-closed, .cancel').on('click', function () {
        $('.popup-wraper8').removeClass('active');
        return false;
    });
// updateImg
    $('#saveImg').on('click', function () {
        $('.popup-wraper10').addClass('active');
        return false;
    });
    $('.upAvt-closed, .cancel').on('click', function () {
        $('.popup-wraper10').removeClass('active');
        return false;
    });
//changeInfor
    $('.accept-change').on('click', function () {
        $('.popup-wraper9').addClass('active');
        return false;
    });
    $('.acceptChange-closed, .cancel').on('click', function () {
        $('.popup-wraper9').removeClass('active');
        return false;
    });
//UpBackground
    $('#upBGR').on('click', function () {
        $('.popup-wraper11').addClass('active');
        return false;
    });
    $('.upBgr-closed, .cancel').on('click', function () {
        $('.popup-wraper11').removeClass('active');
        return false;
    });
// comments popup
...
//counter for funfacts
    ...
    jQuery(".post-comt-box textarea").on("keydown", function (event) {
        ...
        if (event.keyCode == 13) {
            //Lay duoc 3 thong tin
            //postID, userID, detail de truyen vao
            //Dung DOM trong jquery
            var $form = $(this).parent();
            var thisDivItem = $(this).parents(".central-meta.item");
            var spanCommentNum = thisDivItem.find("span.comment");
            var commentNumTag = spanCommentNum.find("ins");
            ...
            $.ajax({
                ...
                success: function (result) {
                    var ul = $form.parents("ul.we-comet");
                    ul.prepend(result);
                    textarea.val('');
                    //window.alert(postID+"/"+userID+"/"+detail);
                    commentNumTag.html(Number(commentNumTag.html()) + 1);
                }
            });
        }
    });
...
function validate1(param) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data === 'true')
                param.value = 'Đừng hack trang tôi bạn ơi!';
            
            
        }
    });
}
function validate2(param) {
    var txt = param.value;
    $.ajax({
        url: 'validate',
        type: 'post',
